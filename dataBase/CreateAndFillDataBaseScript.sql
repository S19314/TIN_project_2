...
CREATE TABLE IF NOT EXISTS `tin-computer-state`.`Zestaw_Elementow_Komputera`
	( `_id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
	  ...
	  `procent_Wykorzystanych_Zasobow` DECIMAL(10,2) NOT NULL,
	  `aktualna_Szybkosc_Przekazania_Danych` DECIMAL(10,2) NOT NULL,
	  `typPolaczenia` VARCHAR(100) NOT NULL,
	  `element_id` INT UNSIGNED NOT NULL,
	  `computer_id` INT UNSIGNED NOT NULL,
	  PRIMARY KEY (`_id`),
	  UNIQUE INDEX `zestaw_id_UNIQUE` (`_id` ASC),
	  CONSTRAINT `element_fk` FOREIGN KEY (`element_id`) REFERENCES `tin-computer-state`.`Element_komputera` (`_id`),
